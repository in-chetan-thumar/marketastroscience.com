...
class LoginController extends Controller
{
    ...
    public function resetPassword(ChangePasswordRequest $request)
    {
        ...
        try {
            ...
            //Match user old password
            if(!Hash::check($request->old_password , $user->password)) {
                return response(['errors' => "Old password is not valid."], 422);
            }
            if($request->old_password == $request->password) {
                return response(['errors' => "New passwords must not be same as old password."], 422);
            }
            //Rest new password
            $user = resolve('user-repo')->changePassword($params, $user->id);
            if ($user) {
                return response(['message' => "Password changed successfully."], 200);
            } else {
                return response(['errors' => "Error in updating password."], 422);
            }
        } catch (\Exception $e) {
            return response(['message' => app('common-helper')->generateErrorMessage($e)], 422);
        }
