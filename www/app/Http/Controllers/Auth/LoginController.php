    ...
    */
    ...
    protected function validateLogin(Request $request)
    {
        ...
        if (config('constants.EMAIL_OTP_LOGIN')) {
            ...
            $messages['username.required'] = 'The email is required.';
            $messages['username.email'] = 'Please provide a valid email address.';
        } elseif (config('constants.MOBILE_OTP_LOGIN')) {
            $rules['username'] = 'required|numeric|digits:10';
            $messages['username.required'] = 'The mobile number is required.';
            $messages['username.numeric'] = 'The mobile number must contain only numbers.';
            $messages['username.digits'] = 'The mobile number must be exactly 10 digits.';
        } else {
    ...
    public function login(Request $request)
    {
        ...
        if ($user) {
            ...
            if (empty($login_attempt) && empty($login_attempt['error'])) {
                // If credentials are correct, generate OTP and redirect to OTP verification page
